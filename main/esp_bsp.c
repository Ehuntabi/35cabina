...
#include "driver/i2c_master.h"   /* nuevo driver — el legacy se atascaba en ets_delay_us ISR */
...
static const axs15231b_lcd_init_cmd_t lcd_init_cmds[] = {
    ...
    {0x21, (uint8_t []){0x00}, 0, 0},   /* INVON: panel arranca con inversion invertida; sin esto fondo negro sale verde claro */
...
/* Devuelve el handle del bus I2C (touch GT911 y otros perifericos). */
...
esp_err_t bsp_i2c_init(void)
{
    ...
    /* Driver NUEVO (driver/i2c_master.h). El legacy (driver/i2c.h) tenia un
     * bug de busy-wait en el ISR (ets_delay_us) que colgaba el chip
     * intermitentemente cuando el slave no respondia como esperaba (INT WDT
     * timeout). El nuevo driver usa otra arquitectura sin esa trampa. */
    const i2c_master_bus_config_t i2c_conf = {
        ...
        .glitch_ignore_cnt = 7,        /* recomendado por IDF */
        .flags.enable_internal_pullup = true,  /* red de seguridad si el board no trae pullups externos */
    };
    BSP_ERROR_CHECK_RETURN_ERR(i2c_new_master_bus(&i2c_conf, &s_i2c_bus));
...
static bool bsp_display_sync_cb(void *arg)
{
    ...
    if (tear_handle->te_v_sync_sem) {
        /* RESTAURADO upstream: portMAX_DELAY. El timeout finito que yo puse
         * (500ms) era la causa del hang LCD: si TE tarda >500ms, retorna
         * "success" falso, draw_bitmap procede mientras DMA del frame previo
         * sigue en vuelo, bus SPI queda lockeado para siempre. El upstream
         * espera infinito porque TE SIEMPRE llega en hardware bueno. */
...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    /* Revertido al macro original (queue=10, pclk=40MHz) tras comparar con
     * el proyecto upstream. El hang LCD no era por queue depth como
     * pensaba; venia de una regresion en sdkconfig
     * (SPIRAM_MALLOC_ALWAYSINTERNAL 16384 -> 4096) que mandaba allocations
     * LVGL a PSRAM, causando contention/race con SPI DMA. */
...
esp_err_t bsp_touch_new(const bsp_display_cfg_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    /* En el driver nuevo el scl_speed va por device (no por bus). El macro
     * deja el campo a 0 — explicito a 400kHz para que i2c_master_bus_add_device
     * acepte el device. */
    tp_io_config.scl_speed_hz = BSP_I2C_CLK_SPEED_HZ;

    /* Usar el handle del bus nuevo (driver/i2c_master.h). El cast a port
     * number era la API legacy. */

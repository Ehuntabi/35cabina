 ...
 *
 */
/* RESTAURADO upstream: task_affinity = -1. Pinear LVGL al core 1 junto con
 * la Tear task (display.h) creaba contention que era parte del hang LCD
 * (race en TE sync). Scheduler distribuye mejor sin pinning. */
...
/**
 * @brief Contador de vida de la tarea LVGL.
 *
 * Avanza tras cada lv_timer_handler(). Si se queda fijo, la tarea LVGL esta
 * colgada (p.ej. en un flush). El supervisor lo usa para reiniciar.
 *
